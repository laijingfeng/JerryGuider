...
from config import Config, ExeRsp
...
def FindTables(use_type):
    ...
    for filename in list:
        if filename.find('.xlsx') != -1:
            if filename.startswith('~$') == True: # 开着的临时文件
                logger.warn('Table ' + filename.split('$', 1)[1] + ' is open, do not forget to do save')
                continue
    ...
    def run(self):
        while True:
            if self.flag == self.F_GEN_PROTO_PY:
                ...
                if ret.returncode != 0:
                    SetError(ret.stderr)
            elif self.flag == self.F_PROTO_TO_CS:
                data = self.que.get()
...
def RunAsync(use_type):
    ...
    os.chdir(config.proto_path)
    for i in range(thread_cnt2):
        t = ThreadHandleCommon(que_common_cs, ThreadHandleCommon.F_PROTO_TO_CS)
        t.setDaemon(True)
        t.start()
...
def CheckConfigPath():
    CheckOneConfigPath(config.table_path)
    CheckOneConfigPath(config.proto_path)
    
    CheckOneConfigPath(config.table_cs_path)
    CheckOneConfigPath(config.table_data_path)
    CheckOneConfigPath(config.common_cs_path)
    CheckOneConfigPath(config.command_cs_path)

    CheckOneConfigPath(config.unity_table_cs_path)
    CheckOneConfigPath(config.unity_table_data_path)
    CheckOneConfigPath(config.unity_common_cs_path)
    CheckOneConfigPath(config.unity_command_cs_path)

def CheckOneConfigPath(check_path):
...
if __name__ == '__main__':
    ...
    logger.info('finish useTime:' + str(time.time() - start_time))
